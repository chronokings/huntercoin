...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    if (wtx.IsGameTx())
    {
        // Note: this code is dependent on the exact structure of transactions generated by the game.
        // Namely, in reward transactions for every tx-out there should be a Null tx-in with info in the scriptSig.
        for (int i = 0; i < wtx.vin.size(); i++)
        {
            const CTxIn &txin = wtx.vin[i];
            if (txin.prevout.IsNull())
            {
                if (i < wtx.vout.size() && wallet->IsMine(wtx.vout[i]))
                    parts.append(TransactionRecord(hash, nTime, TransactionRecord::GameReward,
                                GetGameTxDescription(txin.scriptSig, true), 0, wtx.vout[i].nValue));
            }
            else
            {
                ...
                if (mi != wallet->mapWallet.end())
                {
                    ...
                    if (nout < prev.vout.size() && hooks->IsMine(prev, prev.vout[nout]))
                    {
                        parts.append(TransactionRecord(hash, nTime, TransactionRecord::GameDeath,
                                    GetGameTxDescription(txin.scriptSig, true), 0, 0));
                    }
                }
            }
        }
