...
static QString GetGameTxDescriptionHTML(const CScript &scriptSig)
{
    return QString::fromStdString(
            GetGameTxDescription(scriptSig, false, "<b>", "</b>", false)
        );
}
            ...
            for (int i = 0; i < wtx.vin.size(); i++)
            {
                ...
                if (txin.prevout.IsNull())
                {
                    if (i < wtx.vout.size() && wallet->IsMine(wtx.vout[i]))
                        strHTML += GetGameTxDescriptionHTML(txin.scriptSig) + "<br>";
                }
                else
                {
